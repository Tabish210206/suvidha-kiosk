...
export default function MunicipalGrievance() {
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      const randomId =
        "MUNI-GRV-" + Math.floor(100000 + Math.random() * 900000);

      setComplaintId(randomId);
      setLoading(false);
      setSubmitted(true);

      speak("Municipal grievance submitted successfully");

      // 🔔 GLOBAL NOTIFICATION
      showNotification(
        `Municipal grievance submitted successfully. Complaint ID: ${randomId}`
      );
    }, 2000);
  };
    ...
    <div className="p-10 max-w-3xl mx-auto">
      {!submitted && (
        <>
          ...
          <textarea
            placeholder="Describe your civic issue (waste, drainage, road, etc.)..."
      ...
      {submitted && (
        <div className="bg-white border border-gray-300 rounded-xl p-6 shadow-sm text-center">
          <h3 className="text-xl font-semibold text-green-700 mb-4">
            Municipal Complaint Submitted Successfully
          </h3>
          ...
          <p className="text-gray-700 mb-6">
            Municipal department will review your grievance within
            2–5 working days.
          </p>
